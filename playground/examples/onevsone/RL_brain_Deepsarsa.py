...
class DeepSarsa:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('Sarsa_value'):
            self.name_scope = tf.get_variable_scope().name
            with tf.variable_scope('Sarsa_eval_net_1'):
                c_names, n_l1, w_initializer, b_initializer = \
                    ['Sarsa_eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 50, \
                    tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)  

                
                with tf.variable_scope('Sarsa_l_1'):
                ...
                with tf.variable_scope('Sarsa_hl_1'):
                ...
                with tf.variable_scope('Sarsa_l_2'):
            ...
            with tf.variable_scope('Sarsa_loss'):
                self.loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.q_eval))
            with tf.variable_scope('Sarsa_train'):
                self._train_op = tf.train.RMSPropOptimizer(self.lr).minimize(self.loss)

            # ------------------ build target_net ------------------
            self.s_ = tf.placeholder(tf.float32, [None, self.n_features], name='Sarsa_s_1_')    
            with tf.variable_scope('Sarsa_target_net_1'):
