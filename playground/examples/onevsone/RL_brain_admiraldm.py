    ...
    def _build_net(self):
        ...
        with tf.variable_scope('AdmiralValue'):
            ...
            with tf.variable_scope('Advisorq_eval_net'):
                ...
                with tf.variable_scope('Advisorq_l2'):
            ...
            with tf.variable_scope('Advisorq_loss'):
                self.loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.q_eval))
            with tf.variable_scope('Advisorq_train'):
                self._train_op = tf.train.RMSPropOptimizer(self.lr).minimize(self.loss)

            # ------------------ build target_net ------------------
            self.s_ = tf.placeholder(tf.float32, [None, self.n_features+3], name='Advisorq_s_')    
            with tf.variable_scope('Advisorq_target_net'):
